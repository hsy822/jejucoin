...
#if defined(NDEBUG)
# error "Jejucoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x38e6c280d1ca6dda0c46d4dae1cbf8608f765edb358f227f3fa5bbb51f145f1b");
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Jejucoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Jejucoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Jejucoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Jejucoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Jejucoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7e6ef83b3610dcb575cdfee4925cf59b8444ec68e2c829b9c08ebf9ce6fadc26");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "made by sooyoung hyun";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045dd2a6b0e677b8e2a1efc55c8e1c288ce0b723f2d48b695e3030ed15ddaf185c120357375ca431073169378119684ca70096a348a6e02fbd3f9fdc9e7730c7d4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1525682243;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085084767;

        if (fTestNet)
        {
            block.nTime    = 1525682226;
            block.nNonce   = 385859734;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xce5c8bced4aef9835136b8e21a5a9e32f2288a96d855cfdf2cbe688da37f197b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Jejucoin: increase each by adding 2 to bitcoin's value.
...
void static JejucoinMiner(CWallet *pwallet)
{
    printf("JejucoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("jejucoin-miner");
    ...
    try { loop {
        ...
        printf("Running JejucoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JejucoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JejucoinMiner, pwallet));
}
